...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(`${apiBaseURL()}/api/v1/access/invitations/accept`, {
    method: "POST",
  ...
  if (!token) {
    return NextResponse.json({error: {code: "session_error", message: "Invitation service returned no access token"}}, {status: 502});
  }
  ...
  return NextResponse.json({authenticated: true, created_user: body.created_user, expires_at: body.expires_at, user: body.user}, {headers: {"Cache-Control": "no-store"}});
}
