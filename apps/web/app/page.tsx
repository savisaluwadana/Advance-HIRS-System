...
export default function Home() {
  ...
  async function loadPortal() {
    ...
    try {
      ...
      setLeaveEmployee((current) => current || employeeResponse.data?.[0]?.id || "");
      setAttendanceEmployee((current) => current || employeeResponse.data?.[0]?.id || "");
      setError("");
    } catch (err) {
      const message = String(err);
      if (message.includes("Sign in") || message.includes("unauthorized") || message.includes("401")) setData(emptyData);
  ...
  async function login(event: FormEvent) {
    ...
    try {
      await requestJSON("/api/session/login", {
        ...
        body: JSON.stringify({email, password, organization_slug: organization}),
      });
      setPassword("");
      await loadPortal();
    } catch (err) {
  ...
  async function createLeave(event: FormEvent) {
    event.preventDefault();
    if (!data.user) return;
    setBusy("leave-create");
    try {
      const needsEmployee = data.user.role === "admin" || data.user.role === "hr";
      await requestJSON("/api/hr/leave/requests", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({employee_id: needsEmployee ? leaveEmployee : "", leave_type: leaveType, start_date: leaveStart, end_date: leaveEnd, reason: leaveReason}),
      });
  ...
  async function decideLeave(id: string, decision: "approved" | "rejected") {
    ...
    try {
      await requestJSON(`/api/hr/leave/requests/${id}/decision`, {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({decision, note: `${decision === "approved" ? "Approved" : "Rejected"} from web portal`}),
      });
      await loadPortal();
    } catch (err) { setError(String(err)); }
    finally { setBusy(""); }
  ...
  async function attendanceAction(action: "check-in" | "check-out") {
    ...
    try {
      ...
      await requestJSON(`/api/hr/attendance/${action}`, {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({employee_id: needsEmployee ? attendanceEmployee : "", work_mode: workMode}),
      });
      await loadPortal();
    } catch (err) { setError(String(err)); }
    finally { setBusy(""); }
  ...
  const myAttendance = data.attendance[0];
  const metrics = data.dashboard ? [
    ["People", data.dashboard.employees, "Active workforce"],
    ["Present today", data.dashboard.present_today, "Recorded attendance"],
    ["On leave", data.dashboard.on_leave, "Current leave status"],
    ["Pending leave", data.dashboard.pending_leave_requests, "Needs review"],
  ] : [
    ["My requests", data.leave.length, "Leave history"],
    ["Pending", pending.length, "Awaiting decision"],
    ["Attendance", myAttendance ? "Recorded" : "Not yet", "Today"],
    ["Work mode", myAttendance?.work_mode || "—", "Today"],
  ];

  const approvable = pending.filter((item) => elevated || (manager && item.employee !== user.display_name));
  const directory = useMemo(() => data.employees.slice(0, 8), [data.employees]);
  ...
  return (
    <main className="portal-shell">
      <aside className="portal-sidebar">
        ...
        <nav>
          <button className="portal-nav active">Overview</button>
          <button className="portal-nav">Leave</button>
          <button className="portal-nav">Attendance</button>
          {(elevated || manager) && <button className="portal-nav">People</button>}
          <button className="portal-nav">Documents</button>
        </nav>
      ...
      <section className="portal-content">
        ...
        <div className="portal-metrics">{metrics.map(([label, value, meta]) => <article className="metric-card" key={String(label)}><div className="metric-label">{label}</div><div className="metric-value">{String(value)}</div><div className="metric-change">{meta}</div></article>)}</div>

        <div className="portal-grid">
          <article className="panel portal-card">
            <div className="panel-heading"><div><h2>Request leave</h2><p>Submit a new leave or remote-work request.</p></div></div>
            <form className="portal-form" onSubmit={createLeave}>
              ...
              <label>Reason<textarea value={leaveReason} onChange={(e) => setLeaveReason(e.target.value)} placeholder="Optional context" /></label>
              <button className="primary-button" disabled={busy === "leave-create"}>{busy === "leave-create" ? "Submitting…" : "Submit request"}</button>
            </form>
          </article>

          <article className="panel portal-card">
            <div className="panel-heading"><div><h2>Attendance</h2><p>Record today’s work session.</p></div><span className={`portal-pill ${myAttendance ? "ok" : ""}`}>{myAttendance ? "Recorded" : "Not recorded"}</span></div>
            ...
            <div className="attendance-summary"><span>Today</span><strong>{myAttendance?.check_in ? new Date(myAttendance.check_in).toLocaleTimeString([], {hour: "2-digit", minute: "2-digit"}) : "—"} → {myAttendance?.check_out ? new Date(myAttendance.check_out).toLocaleTimeString([], {hour: "2-digit", minute: "2-digit"}) : "—"}</strong><small>{myAttendance?.work_mode || "No work mode recorded"}</small></div>
          </article>
        </div>

        {(elevated || manager) && <article className="panel portal-card portal-wide">
          <div className="panel-heading"><div><h2>Leave approvals</h2><p>{manager ? "Pending requests from your team." : "Pending requests across the organization."}</p></div><span className="portal-pill">{approvable.length} pending</span></div>
          <div className="approval-list">{approvable.length === 0 ? <div className="portal-empty">Nothing needs approval.</div> : approvable.map((item) => <div className="approval-row" key={item.id}><div><strong>{item.employee}</strong><p>{item.type} · {item.start} → {item.end} · {item.days} day{item.days === 1 ? "" : "s"}</p></div><div className="approval-actions"><button onClick={() => decideLeave(item.id, "approved")} disabled={busy === `leave-${item.id}`}>Approve</button><button className="danger" onClick={() => decideLeave(item.id, "rejected")} disabled={busy === `leave-${item.id}`}>Reject</button></div></div>)}</div>
        </article>}

        <div className="portal-grid lower-grid">
          <article className="panel portal-card">
            <div className="panel-heading"><div><h2>Leave history</h2><p>Your visible request history.</p></div></div>
            <div className="history-list">{data.leave.slice(0, 7).map((item) => <div className="history-row" key={item.id}><div><strong>{item.employee}</strong><p>{item.type} · {item.start} → {item.end}</p></div><span className={`status-badge ${item.status}`}>{item.status}</span></div>)}</div>
          </article>

          {(elevated || manager) && <article className="panel portal-card">
            <div className="panel-heading"><div><h2>People</h2><p>{manager ? "Organization directory." : "Workforce directory."}</p></div><span>{data.employees.length}</span></div>
            <div className="directory-list">{directory.map((employee) => <div className="directory-row" key={employee.id}><span className="avatar soft">{initials(employee.name)}</span><div><strong>{employee.name}</strong><p>{employee.role} · {employee.department}</p></div><span className={`status-badge ${employee.status}`}>{employee.status}</span></div>)}</div>
          </article>}
        </div>
      </section>
    </main>
